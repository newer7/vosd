...
def dataload(args, augmentations, normalize, data_info):
    if args.dataset == 'CIFAR10':
        train_dataset = datasets.CIFAR10(
            root=args.datapath, train=True, download=True, transform=transforms.Compose([
            transforms.Resize(data_info['img_size'], interpolation=Image.BICUBIC),augmentations]))
        
        val_dataset = datasets.CIFAR10(
            root=args.datapath, train=False, download=True, transform=transforms.Compose([
            transforms.Resize(data_info['img_size'], interpolation=Image.BICUBIC),
            transforms.ToTensor(),
            *normalize]))
