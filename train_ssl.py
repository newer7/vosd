...
def get_args_parser():
    ...
    parser.add_argument('--patch_size', default=4, type=int,
                        help="""Size in pixels of input square patches - default 4 (for 4x4 patches) """)
    parser.add_argument('--out_dim', default=1024, type=int,
                        help="""Dimensionality of the SSL MLP head output. For complex and large datasets large values (like 65k) work well.""")
    ...
    parser.add_argument('--use_mean_pooling', default=False, type=bool,
                        help=""" Shared Relative position embeddings """)
    ...
    parser.add_argument('--dataset', default='CIFAR10', type=str,
                        choices=['Tiny-Imagenet', 'CIFAR10', 'CIFAR100', 'CINIC', 'SVHN', 'Aircraft', 'Cars'],
...
def train(args):
    ...
    filtered_student_state_dict = {k: v for k, v in student.module.state_dict().items() if
                                   k in teacher_without_ddp.state_dict().keys()}
...
class DataAugmentation(object):
    def __init__(self, args):
        ...
        self.local_crops_number = args.local_crops_number  # 裁剪的数量：8
        self.local_transfo = transforms.Compose([
            transforms.RandomResizedCrop(args.image_size // 2, scale=args.local_crops_scale,
